...
- **Location:** `~/.config/nvim-copilot/`
- **Runtime data:** `~/.local/share/nvim-copilot/`
- **Launch:** `NVIM_APPNAME=nvim-copilot nvim`
- **Neovim version:** 0.12+
...
- Plugins are cloned into `~/.local/share/nvim-copilot/site/pack/<group>/start/<name>/`
...
```bash
clone "<pack-group>" "<github-user/repo>" "<directory-name>" ["<branch>"]
```

The optional 4th argument pins a specific branch (e.g. `"stable"`).
...
lua/
  ...
  plugins/
    ...
    snacks.lua              # folke/snacks.nvim (includes explorer, pickers, dashboard, etc.)
```
...
- **fyler.nvim** — no longer used; file explorer is snacks.nvim built-in explorer
...
4. Remove the installed directory: `rm -rf ~/.local/share/nvim-copilot/site/pack/<group>/start/<name>`
